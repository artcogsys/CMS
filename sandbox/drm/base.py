...
class DRMNet(ChainList, Network):
    ...
    def __call__(self, x, train=False):
        ...
        pop_output = [Variable(np.zeros([x.shape[0], p.n_output]).astype('float32')) for p in self.populations]
        ...
        #
        # # initialize population outputs
        # pop_output = [Variable(np.zeros(p.n_output).astype('float32')) for p in self.populations]
        #
        # # for each population append the inputs generated by the other populations
        # # we do this in a random order. Populations cannot depend on receiving input
        # # in the right order. Instead, they need to incorporate some mechanism to
        # # integrate incoming input over time.
        #
        # # compute input to each neural population
        # for i in range(self.n_pop):
        #     pop_input=[]
        #     wp = self.Wp[i]
        #     for j in range(self.n_pop):
        #         if not wp[j] is None:
        #             pop_input.add(wp[j](pop_output[j]))
