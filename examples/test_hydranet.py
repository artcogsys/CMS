# example of life-long learning with hydranet

import chainer
from chainer import Chain
import chainer.functions as F
from models.links import Elman
import chainer.links as L
from learners.iterators import *
from learners.supervised_learner import StatefulTrainer
from models.models import Regressor
from chainer.datasets import *

#####
## Hydranet RNN

class HydraRNN(Chain):

    def __init__(self, n_input, n_hidden):

        super(HydraRNN, self).__init__(
            l1=Elman(n_input, n_hidden),
            l2=L.Linear(n_hidden, n_input),
        )

    def __call__(self, x, train=False):

        h = self.l1(F.dropout(x, train=True))
        y = self.l2(h)

        return y

    def reset_state(self):
        self.l1.reset_state()

#####
## Hydranet iterator

class HydraIterator(DataIterator):

    def __init__(self, batch_size=32, n_batches=None, n_update=1000):
        super(HydraIterator, self).__init__(get_mnist()[0]._datasets[0].astype('float32'))

        self.batch_size = batch_size # 1 is most reasonable for realistic scenarios

        self.n_batches = n_batches or np.inf

        self.n_update = n_update # update task after so many iterations

        self._order = []

        self.n_input = self.data[0].size

    def __iter__(self):

        self.batch_idx = 0

        return self

    def next(self):

        # never stop iterating during life-long learning
        if self.batch_idx == self.n_batches:
            raise StopIteration

        if self.batch_idx % self.n_update == 0:
            self._order = np.random.permutation(len(self.data))[0:self.batch_size]

        self.batch_idx += 1

        return [tuple([self.data[index], self.data[index]]) for index in self._order]


#####
## Main

# parameters
n_epochs = 10

# iterator
iter = HydraIterator(batch_size=32)

# define model
model = Regressor(HydraRNN(iter.n_input, n_hidden=10))

# Set up an optimizer
optimizer = chainer.optimizers.Adam()
optimizer.setup(model)
optimizer.add_hook(chainer.optimizer.WeightDecay(1e-5))

# define trainer object
trainer = StatefulTrainer(optimizer, iter, cutoff=50)

trainer.run(plot=True)



